...
function qfAttrs(head){const m=head.match(/\{([^}]*)\}/);const attrs={raw:m?m[1]:''};if(!m)return attrs;for(const kv of m[1].matchAll(/([\w-]+)="([^"]*)"/g))attrs[kv[1]]=kv[2];attrs.front=/\.layout-front\b/.test(m[1]);attrs.two=/\.layout-1-1\b/.test(m[1]);attrs.dark=(attrs.background||'').toLowerCase()==='#0f1e23'||/scientific-dark/.test(m[1]);return attrs}
function qfCleanHeading(line){return line.replace(/^#{1,6}\s+/,'').replace(/\s*\{[^}]*\}\s*$/,'').replace(/[\*_`]/g,'').trim()}
function qfImageSrc(path){const name=path.split('/').pop();return qfPreviewUrls.get(name)||''}
function qfMdInline(s){return qfEsc(s).replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\*(.*?)\*/g,'<em>$1</em>').replace(/`([^`]+)`/g,'<code>$1</code>')}
function qfRegion(slide,name){const rx=new RegExp(`:::\\s*${name}[^\\n]*\\n([\\s\\S]*?)\\n:::`,`i`);const m=slide.match(rx);return m?m[1].trim():''}
